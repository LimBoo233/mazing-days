...
namespace Modules.Combat.FSM.BattleState
{
	public class ExecuteSkillState : IBattleState
	{
		...
		public void Enter(CombatManager combatManager)
		{
			...
			SkillDataSo skill = attacker.Data.Skills.Find(skill => skill.SkillID == combatManager.SelectedSkillId);
		...
		private void ExecuteSkillOnTarget(Unit attacker, Unit target, SkillDataSo skill)
		{
			...
			{
				//进行攻击掷投判断
				AttackResult attackResult = CombatResolver.RollAttackValue(
					attacker.Data.BaseAttackModifier,
					target.Data.DexModifier,
					attacker.Data.CriticalNeed
				);
				...
				logDetail =
					$"[{resultStr}] 掷骰:{attackResult.RawRoll} + {attacker.Data.BaseAttackModifier} = {attackResult.FinalValue} (VS AC {target.Data.Ac})";
			}
