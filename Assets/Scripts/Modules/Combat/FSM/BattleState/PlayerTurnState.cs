...
namespace Modules.Combat.FSM.BattleState
{
	public class PlayerTurnState : IBattleState
	{
		...
		private void UpdateChooseSkill(CombatManager combatManager)
		{
			...
			{
				_selectedSkill = combatManager.CurrentActiveUnit.Data.Skills.Find
		...
		{
			// 反悔选择技能
			if (GameManager.Input.IsCancelSelected())
			{
				EnterChooseSkillPhase(combatManager);
				return;
			}
			...
			// 尝试选中敌人
			if (GameManager.Input.IsTrySelected())
			{
				...
				switch (_selectedSkill.TargetType)
				{
					...
					case TargetType.SingleEnemy:
						if (!target.Data.IsDead && target.Data.FactionType == FactionType.Enemy)
						{
					...
					case TargetType.AllEnemies:
						if (target.Data.FactionType == FactionType.Enemy)
						{
					...
					case TargetType.SingleAlly:
						if (!target.Data.IsDead && target.Data.FactionType == FactionType.Player)
						{
							finalTarget.Add(target);
							isValidSelection = true;
						}
					...
					case TargetType.AllAllies:
						if (target.Data.FactionType == FactionType.Player)
						{
